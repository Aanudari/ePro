...
function RatingIndividual() {
    ...
    const handleSubmit = (category, total, itemId) => {
        let pre = schema.categories.map((item) => {
            return (item.id == itemId) ? ({ ...item, subCategory: category, points: total }) : item
        })
    ...
    let final = mainTotal.map(e => Number(e)).reduce((a, b) => Math.round((a + b)), 0)
    ...
    return (
        ...
        bg-black bg-opacity-50 flex items-center justify-center z-10`}>
                    <div className="bg-white w-6/12  rounded-t ">
                        ...
                        <div className="p-2 font-[400]">Үнэлгээ хадгалах бол "Тийм" товчыг дарна уу ?</div>
                        <div className=" flex justify-end px-2 py-2">
                            <button className="p-2 border px-4 mx-1 my-1 rounded font-[400]">Тийм</button>
                            <button className="p-2 border px-4 mx-1 my-1 rounded font-[400]">Үгүй</button>
                        </div>
                    </div>
                </div>
            }
            ...
            <div className="w-full h-full relative">
                <div className="h-14 relative">
                    <div className={` ${activeMenu ? "w-[calc(100%-250px)]" : 'w-full'} fixed h-14 bg-gray-100 border-b-[2px] border-gray-300 shadow-sm flex justify-between
        items-center px-4`}>
                ...
                <div className="px-4 pt-2 pb-20">
                    {
                        template.categories.map((item, index) => (
                            <RatingProcess item={item} key={index} handleSubmit={handleSubmit} />
                        ))
                    }
                </div>
                <div className="h-14  fixed bottom-0 right-0 flex items-center px-4 justify-end">
                    <div className="h-9 shadow-lg bg-teal-500 rounded-sm pl-3 flex items-center font-[400] text-white

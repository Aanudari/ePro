      ...
      <div className="h-full flex">
        <div className="w-full p-3 bg-gray-100">
          ...
          {rate
            ? rate.map((data, index) => (
                <div
                  key={index}
                  className="mt-3 w-full rounded-lg bg-white p-2 fw-bold"
                >
                  <h5>TOTAL 1/2 үнэлгээний нийт хувь: {data.rating}%</h5>
                  <div className="flex flex-wrap">
                    <div className="mt-3 w-full lg:w-6/12 xl:w-4/12 px-2">
                      <div className="relative flex flex-col min-w-0 break-words bg-white rounded mb-6 xl:mb-0 shadow-sm">
                        <div className="flex-auto p-4">
                          <div className="flex flex-wrap">
                            <div className="relative w-full pr-4 max-w-full flex-grow flex-1">
                              <h5 className="text-blueGray-400 uppercase font-bold text-xs">
                                Үнэлгээг үүсэгсэн огноо:
                              </h5>
                  ...
                  <div className="grid gap-6 mb-6 md:grid-cols-3 mt-5 px-2 border-2">
                    {data
                      ? data.extras.map((data, index) => (
                          <div key={index}>
                            <a className="text-black">
                              {data.name}: <p> {data.value}</p>
                            </a>
                          </div>
                        ))
                      : null}
                  ...
                  {data
                    ? data.categories.map((data, index) => (
                        <div key={index}>
                          <h6 className="border-2">
                            Ур чадварын нэр: {data.name}
                          </h6>
                          <table
                            id="example"
        ...
        <div className="flex">
          <div
            className={`${
              open ? "w-72" : "w-20"
            } bg-white h-screen p-1 pt-3 relative duration-300`}
          >
            <i
              className={`bi bi-arrow-right-circle-fill absolute cursor-pointer-left-2 top-10 w-10 border-black-300
           border-4 rounded-full ${!open && "rotate-180"}`}
            ...
            <div className="flex gap-x-4 items-center">
              ...
              />
              <h5
                className={`text-black origin-left font-medium text-sm duration-200 ${
                  !open && "scale-0"
                }`}
              >
                Хэрэглэгчийн цэс
            ...
            <ul className="pt-10 pl-3">
              <li
                className={`flex rounded-md p-2 cursor-pointer hover:bg-blue text-gray-500 text-sm 
                                    items-right gap-x-3 bg-light-white`}
              >
                <i className="bi bi-alarm" />
                <span
                  className={`${!open && "hidden"} origin-right duration-300`}
                >
                  <SelectRate

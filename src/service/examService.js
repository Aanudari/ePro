...
export const getCategoryPool = async () => {
  const response = await axios.get(
    `${process.env.REACT_APP_URL}/v1/Pool/Category`,
    {
      headers: {
...
export const getQuestionDetail = async () => {
  const response = await axios.get(
    `${process.env.REACT_APP_URL}/v1/Exam/Variants/${localStorage.getItem(
      "exam_id"
    )}`,
    {
      headers: {
...
export const addCategoryPool = async (data) => {
  const response = await axios.post(
    `${process.env.REACT_APP_URL}/v1/Pool/add/Category`,
    data,
    {
      headers: {
        Authorization: `${localStorage.getItem("token")}`,
...
export const deleteCategoryPool = async (data) => {
  const response = await axios.delete(
    `${process.env.REACT_APP_URL}/v1/Pool/${data}`,
    {
      headers: {
        Authorization: `${localStorage.getItem("token")}`,
        "Content-Type": "application/json",
